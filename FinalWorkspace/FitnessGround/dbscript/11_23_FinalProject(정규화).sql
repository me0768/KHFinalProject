...
DROP TABLE TB_USER_AUTHORITY CASCADE CONSTRAINTS;
PROMPT CREATING USER_AUTHORITY TABLE..
CREATE TABLE TB_USER_AUTHORITY(
  AUTHORITY_NO NUMBER CONSTRAINT PK_UA PRIMARY KEY,
  AUTHORITY_NAME VARCHAR2(20) NOT NULL
);
COMMENT ON COLUMN TB_USER_AUTHORITY.AUTHORITY_NO IS '유저권한번호';
COMMENT ON COLUMN TB_USER_AUTHORITY.AUTHORITY_NAME IS '유저권한이름';
ALTER TABLE TB_USER
  ADD CONSTRAINT FK_USER FOREIGN KEY(USER_LEVEL) REFERENCES TB_USER_AUTHORITY;
...
CREATE TABLE TB_CATEGORY(
  CATEGORY_NO NUMBER CONSTRAINT PK_C PRIMARY KEY,
  CATEGORY_NAME VARCHAR2(100) NOT NULL
);
COMMENT ON COLUMN TB_CATEGORY.CATEGORY_NO IS '카테고리 번호';
COMMENT ON COLUMN TB_CATEGORY.CATEGORY_NAME IS '카테고리 이름';
ALTER TABLE TB_VIDEO_BOARD
  ADD CONSTRAINT FK1_VB FOREIGN KEY(CATEGORY1) REFERENCES TB_CATEGORY(CATEGORY_NO);
ALTER TABLE TB_VIDEO_BOARD
  ADD CONSTRAINT FK2_VB FOREIGN KEY(CATEGORY2) REFERENCES TB_CATEGORY(CATEGORY_NO);
...
CREATE TABLE TB_COM_BOARD(
  ...
  TITLE VARCHAR2(200) NOT NULL,
  CONTENT VARCHAR2(4000) NOT NULL,
  BOARD_PROPERTY NUMBER NOT NULL,
  UPLOAD_DATE DATE DEFAULT SYSDATE,
  READCOUNT NUMBER DEFAULT 0
);
...
DROP TABLE TB_BOARD_PROPERTY CASCADE CONSTRAINTS;
PROMPT CREATING BOARD_PROPERTY TABLE..
CREATE TABLE TB_BOARD_PROPERTY(
  PROPERTY_NO NUMBER CONSTRAINT PK_BP PRIMARY KEY,
  PROPERTY_NAME VARCHAR2(20) NOT NULL
);
COMMENT ON COLUMN TB_BOARD_PROPERTY.PROPERTY_NO IS '게시판특성 번호';
COMMENT ON COLUMN TB_BOARD_PROPERTY.PROPERTY_NAME IS '게시판특성 이름';
ALTER TABLE TB_COM_BOARD
  ADD CONSTRAINT FK2_CB FOREIGN KEY(BOARD_PROPERTY) REFERENCES TB_BOARD_PROPERTY;
...
ALTER TABLE TB_QNA
  ADD CONSTRAINT FK1_Q FOREIGN KEY(SENDER) REFERENCES TB_USER;
ALTER TABLE TB_QNA
  ADD CONSTRAINT FK2_Q FOREIGN KEY(RECEIVER) REFERENCES TB_USER;
ALTER TABLE TB_QNA
